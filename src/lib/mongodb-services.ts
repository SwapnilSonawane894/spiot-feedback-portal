...
export const COLLECTIONS = {
  ...
  SETTINGS: 'settings',
  PRINCIPAL_SUGGESTIONS: 'principalSuggestions',
};
...
// ============ PRINCIPAL SUGGESTION OPERATIONS ============

export const principalSuggestionService = {
  async findMany(params?: { where?: any }) {
    try {
      const db = await getDatabase();
      const query: any = params?.where || {};
      const results = await db.collection(COLLECTIONS.PRINCIPAL_SUGGESTIONS).find(query).toArray();
      return results.map(docWithId);
    } catch (error) {
      throw error;
    }
  ...
  async findUnique(where: { hodStaffId_semester?: { hodStaffId: string; semester: string } }) {
    try {
      const db = await getDatabase();
      let query: any = {};
      if (where.hodStaffId_semester) {
        const { hodStaffId, semester } = where.hodStaffId_semester;
        query = { hodStaffId, semester };
      }
      const doc = await db.collection(COLLECTIONS.PRINCIPAL_SUGGESTIONS).findOne(query);
      return docWithId(doc);
    } catch (error) {
      throw error;
    }
  ...
  async upsert(where: { hodStaffId: string; semester: string }, data: any) {
    try {
      const db = await getDatabase();
      const { hodStaffId, semester } = where;
      const result = await db.collection(COLLECTIONS.PRINCIPAL_SUGGESTIONS).findOneAndUpdate(
        { hodStaffId, semester },
        {
          $set: { ...data, hodStaffId, semester, updatedAt: new Date() },
          $setOnInsert: { createdAt: new Date() }
        },
        { upsert: true, returnDocument: 'after' }
      );
...
// ============ HOD REPORT RELEASE SERVICE (controlled by Principal) ============

export const hodReportReleaseService = {
  async getStatus() {
    try {
      const db = await getDatabase();
      const doc = await db.collection(COLLECTIONS.SETTINGS).findOne({ type: 'hodReportReleaseStatus' });
      return { isReleased: doc?.isReleased ?? false };
    } catch (error) {
      throw error;
    }
  },

  async setStatus(isReleased: boolean) {
    try {
      const db = await getDatabase();
      await db.collection(COLLECTIONS.SETTINGS).updateOne(
        { type: 'hodReportReleaseStatus' },
        { $set: { isReleased, updatedAt: new Date() }, $setOnInsert: { type: 'hodReportReleaseStatus', createdAt: new Date() } },
        { upsert: true }
      );
      return { isReleased };
    } catch (error) {
