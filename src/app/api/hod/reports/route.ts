...
export async function GET(req: Request) {
  ...
  const semester = url.searchParams.get('semester');
    ...
    // Exclude the HOD themselves from the faculty list — only principal gives HOD suggestions
    const hodOwnStaffId = String(staff.id);
    const reportsStaffList = staffList.filter((s: any) => String(s.id) !== hodOwnStaffId);

    const reports = await Promise.all(reportsStaffList.map(async (s: any) => {
