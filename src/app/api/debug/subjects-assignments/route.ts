...
import { subjectService, assignmentService, staffService, departmentSubjectsService } from '@/lib/mongodb-services';
...
export async function GET(req: Request) {
  ...
  // Limit to assignments that reference department subjects
  const deptSubjIds = (deptSubjects || []).map((s: any) => s.id);
  const assignments = await assignmentService.findMany({ where: { subjectId: deptSubjIds } });
  const assignmentsForDeptSubjects = assignments.filter((a: any) => a.subjectId && deptSubjIds.some((id: any) => String(id) === String(a.subjectId)));
