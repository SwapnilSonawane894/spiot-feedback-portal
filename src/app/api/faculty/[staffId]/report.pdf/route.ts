...
import { staffService, assignmentService, hodSuggestionService, principalSuggestionService } from "@/lib/mongodb-services";
...
export async function GET(req: Request, ctx: { params?: any }) {
  ...
  const viewerIsPrincipal = viewerRole === 'PRINCIPAL';
  const viewerIsSelf = (viewerRole === 'STAFF' || viewerRole === 'FACULTY') && viewerId === staff.userId;
  const allowed = viewerIsHod || viewerIsSelf || viewerIsPrincipal;
  ...
  // If PRINCIPAL is downloading the HOD's report, fetch principal suggestion instead
  let principalSuggestion: any = null;
  if (viewerIsPrincipal) {
    principalSuggestion = await principalSuggestionService.findUnique({
      hodStaffId_semester: { hodStaffId: staffId, semester }
    });
  }
    ...
    for (const a of assignments) {
      ...
      if (!viewerIsHod && !viewerIsPrincipal) fbWhere.isReleased = true;
  ...
  page.drawText('Overall Performance', { x: tableX + 6, y: overallRowTop - 12, size: 10, font: times });
    ...
    // Choose suggestion source and labels based on viewer role (PRINCIPAL=principal suggestions, others=HOD suggestions)
    const hs = viewerIsPrincipal ? principalSuggestion : hodSuggestion;
    const suggestionHeading = viewerIsPrincipal ? 'Principal Suggestions:' : 'HOD Suggestions:';
    const noSuggestionText = viewerIsPrincipal ? 'No suggestions from Principal yet.' : 'No suggestions from HOD yet.';
    const responseHeading = viewerIsPrincipal ? 'HOD Response:' : 'Faculty Response:';
    const noResponseText = viewerIsPrincipal ? 'No response from HOD.' : 'No response from faculty.';

    y -= 8;
    page.drawText(suggestionHeading, { x: tableX, y, size: 11, font: times });
    y -= 14;
    if (hs && hs.content) {
      ...
      page.drawText(noSuggestionText, { x: tableX + 6, y, size: 9, font: times });
      y -= 14;
    }

    // Response Section (HOD response for principal-view, Faculty response for HOD-view)
    y -= 10;
    if (y < 100) {
      page = pdfDoc.addPage([595.28, 841.89]);
      y = page.getHeight() - 40;
    }
    page.drawText(responseHeading, { x: tableX, y, size: 11, font: times });
    y -= 14;
    
    const responseText = viewerIsPrincipal ? hs?.hodResponse : hs?.facultyResponse;
    if (hs && responseText) {
      // Split response by newlines and create numbered points
      ...
      if (responseLines.length > 0) {
        ...
        page.drawText(noResponseText, { x: tableX + 6, y, size: 9, font: times });
        y -= 14;
      }
    } else {
      page.drawText(noResponseText, { x: tableX + 6, y, size: 9, font: times });
