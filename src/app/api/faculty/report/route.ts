...
export async function GET(req: Request) {
  try {
    ...
    // Accept STAFF, FACULTY, HOD, and PRINCIPAL roles
    const callerRole = session.user?.role;
    if (callerRole !== "STAFF" && callerRole !== "HOD" && callerRole !== "FACULTY" && callerRole !== "PRINCIPAL") {
      return NextResponse.json({ error: "Forbidden" }, { status: 403 });
    }

    // PRINCIPAL has no staff profile — they view other staff by staffId query param
    let staff: any = null;
    if (callerRole !== "PRINCIPAL") {
      staff = await staffService.findFirst({ where: { userId: session.user.id } });
      if (!staff) {
    ...
    if (callerRole === "PRINCIPAL") {
      if (!requestedStaffId) return NextResponse.json({ error: "staffId is required for PRINCIPAL" }, { status: 400 });
      ...
      targetStaff = ts;
    } else if (requestedStaffId && callerRole === "HOD" && requestedStaffId !== staff.id) {
      const ts = await staffService.findFirst({ where: { id: requestedStaffId } });
      if (!ts) return NextResponse.json({ error: "Target staff not found" }, { status: 404 });
      targetStaff = ts;
    ...
    if (debugMode) {
      ...
      return NextResponse.json({
        debug: true,
        staffId: (staff ?? targetStaff)?.id,
        staffDepartmentId: (staff ?? targetStaff)?.departmentId,
    ...
    for (const [deptId, assignments] of assignmentsByDept.entries()) {
      ...
      for (const a of assignments) {
        ...
        // If the viewer is not an HOD or PRINCIPAL, only include feedbacks that have been released by HOD
        const viewerIsHodOrPrincipal = session.user?.role === 'HOD' || session.user?.role === 'PRINCIPAL';
        if (!viewerIsHodOrPrincipal) {
      ...
      const response = NextResponse.json({
        ...
        hodSuggestion,
        homeDepartmentId: (staff ?? targetStaff)?.departmentId, // staff's home department
      });
