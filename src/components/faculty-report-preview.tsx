...
export function DepartmentReportSection({ 
  ...
  const title = supervisorTitle || "HOD";
    ...
    <div className="mb-8">
      ...
      <div className="card">
        <div className="card-body">
          {/* Suggestions Section — hidden when parent handles it separately */}
          {!hideSuggestion && (
            <>
              <div className="mb-6 p-4 rounded-lg" style={{ backgroundColor: "var(--hover-overlay)", borderLeft: "4px solid var(--primary)" }}>
                <div className="flex items-center gap-2 mb-2">
                  <MessageSquare size={18} style={{ color: "var(--primary)" }} />
                  <h3 className="font-medium" style={{ color: "var(--text-primary)" }}>{title} Suggestions</h3>
                </div>
              ...
              <div className="mb-6 p-4 rounded-lg" style={{ backgroundColor: "var(--hover-overlay)", borderLeft: "4px solid var(--success)" }}>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <h3 className="font-medium" style={{ color: "var(--text-primary)" }}>
                    {isHodMode ? `Faculty Response to ${title} Suggestions` : `Your Response to ${title} Suggestions`}
                  </h3>
                </div>
                {!isHodMode && (
                  <p className="text-xs mb-2" style={{ color: "var(--text-muted)" }}>
                    Clarify your comments or action plan based on the {title} suggestions above. Enter each point on a new line - they will be automatically numbered.
                  </p>
                )}
                {isHodMode ? (
                  ...
                  <>
                    <textarea
                      ...
                      placeholder={deptData.hodSuggestion ? "Enter your response (one point per line)..." : `No ${title} suggestion to respond to yet.`}
