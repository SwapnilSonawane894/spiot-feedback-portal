...
async function addOriginalSubjectsWithObjectIds() {
  try {
    ...
    // Create academic year with ObjectId
    const academicYear = {
      _id: new ObjectId(),
      name: '2023-24',
      abbreviation: '23-24',
    ...
    // Create departments with ObjectIds
    const departments = {
      CO: {
        _id: new ObjectId(),
    ...
    // Get fresh department IDs from database
    const dbDepartments = await db.collection('departments').find({}).toArray();
    const departmentMap = {};
    for (const dept of dbDepartments) {
      departmentMap[dept.abbreviation] = dept._id;
    }

    // Add subjects for each department
    for (const [deptCode, yearSubjects] of Object.entries(subjectsByDept)) {
      const deptId = departmentMap[deptCode];
      if (!deptId) {
        console.log(`Warning: Department ${deptCode} not found in database`);
        continue;
      }
...
addOriginalSubjectsWithObjectIds();