...
async function addOriginalSubjectsFixed() {
  try {
    ...
    const academicYearResult = await db.collection('academicYears').insertOne({
      name: '2023-24',
      abbreviation: '23-24',
      year: 2023,
      createdAt: new Date(),
    ...
    const academicYearId = academicYearResult.insertedId;
    ...
    for (const dept of deptData) {
      ...
      departmentIds[dept.abbreviation] = result.insertedId;
    }
    ...
    // Log a sample subject and its mapping for debugging
    const sampleSubject = await db.collection('subjects').findOne({});
    console.log('\nSample subject:', sampleSubject);
    if (sampleSubject) {
      const sampleMapping = await db.collection('departmentSubjects').findOne({ subjectId: sampleSubject._id });
